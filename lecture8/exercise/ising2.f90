...
contains
  ...
  subroutine init_updown( spin_val )
    
    integer :: x, y
    integer, intent(in) :: spin_val
  ...
  subroutine output(cum)
    ...
    real (kind = double), dimension(5), intent (inout) :: cum
    real (kind = double) :: eave, e2ave, mave, m2ave, abs_mave, heat_c, chi
    ...
    heat_c   = ( cum(2) / real(nmcs) - (cum(1)/real(nmcs))**2) / T**2 / real(N)
    chi      = ( cum(4) / real(nmcs) - (cum(3)/real(nmcs))**2) / T / real(N)
...
program ising
  ...
  open(unit=1, file='equilibrium.dat', status='replace')
  ...
  do imcs = 1,nequil
     ...
     if( mod(imcs-1, every) == 0 ) then
        write(1,*) imcs, cum(1)/real(N)/real(imcs), cum(3)/real(N)/real(imcs)
     end if
